...
int SCOproc(const int * pthis, char** database, char** key, char** player, int flags, unsigned char * pData, int size)
{
  if (*pthis)
  {
    ...
    asm ("pusha");
    int lastRet = (unsigned long)db.WriteSCO(*database, *key, *player, flags, pData, size);
    asm ("popa");
    return lastRet;
  }
  return 0;
...
unsigned char * RCOproc(const int * pthis, char** database, char** key, char** player, int* arg4, int* size)
{
  if (*pthis)
  {
    ...
    asm ("pusha");
    unsigned char * lastRet = db.ReadSCO(*database, *key, *player, arg4, size);
    asm ("popa");
    return lastRet;
  }
  return 0;
