/***************************************************************************
    db plugin for NWNX  - hooks implementation
...
char *SaveObject(dword nObjectID, int &nSize)
{
	...
	if(pObject)
	{
		...
		db.Log(4, "Created structures\n");
		CResGFF__CResGFF(pGFF);
		db.Log(4, "CResGFF__CResGFF - done\n");
		if(CResGFF__CreateGFFFile(pGFF, pResStruct, sType, sVersion)){
			db.Log(4, "CResGFF__CreateGFFFile - done\n");
			if(SaveObjectInternal(pObject, pGFF, pResStruct)){
				db.Log(4, "CNWSPlaceable__SavePlaceable - done\n");
				...
				if(CResGFF__WriteGFFToPointer(pGFF, &pData, nDataSize)){
					db.Log(4, "CResGFF__WriteGFFToPointer - done\n");
					db.Log(4, "Length: %d, data: %s\n", nDataSize, pData);
					//free(pData);
...
char *SaveStore(CNWSStore *pObject, int &pnSize)
{
	...
	if(CResGFF__CreateGFFFile(pGFF, pResStruct, sType, sVersion)){
		...
		if(CNWSStore__SaveStore(pObject, pGFF, pResStruct)){
			db.Log(4, "CNWSStore__SaveStore - done\n");
			//TODO: Save object state / Load var table
			//CNWSObject__SaveVarTable(pObject, pGFF, pResStruct);
			void *pData;
			int nDataSize;
...
dword LoadObject(const char *pData, int nSize, Location lLocation)
{
	...
	if(sType == "UTP ")
	{
		...
		if(pArea)
		{
			...
			db.Log(4, "CNWSPlaceable__AddToArea - done\n");
		}
		if(pGFF)
			CResGFF___CResGFF(pGFF, 3);
		delete pResStruct;
	...
	else if(sType == "UTM ")
	{
		...
		CNWSObject__LoadVarTable((CNWSObject *)pObject, pGFF, pResStruct);
		db.Log(4, "CNWSStore__LoadStore - done\n");
		CNWSArea *pArea = CServerExoApp__GetAreaByGameObjectID((void *)pServThis, lLocation.AreaID);
		db.Log(4, "pArea = %08lx\n", pArea);
		db.Log(4, "AreaID = %08lx, area type = %d\n", lLocation.AreaID, pArea->GameObject.ObjectID);
		if(pArea)
		{
			...
			db.Log(4, "CNWSStore__AddToArea - done\n");
		}
		if(pGFF)
			CResGFF___CResGFF(pGFF, 3);
		delete pResStruct;
	...
	else if(sType == "UTT ")
	{
		...
		db.Log(4, "CNWSTrigger__CNWSTrigger - done\n");
		CNWSTrigger__LoadTrigger(pObject, pGFF, pResStruct, NULL);
		CNWSObject__LoadVarTable((CNWSObject *)pObject, pGFF, pResStruct);
		db.Log(4, "CNWSTrigger__LoadTrigger - done\n");
		CNWSArea *pArea = CServerExoApp__GetAreaByGameObjectID((void *)pServThis, lLocation.AreaID);
		db.Log(4, "pArea = %08lx\n", pArea);
		db.Log(4, "AreaID = %08lx, area type = %d\n", lLocation.AreaID, pArea->GameObject.ObjectID);
		if(pArea)
		{
			...
			db.Log(4, "CNWSTrigger__AddToArea - done\n");
		}
		if(pGFF)
			CResGFF___CResGFF(pGFF, 3);
		delete pResStruct;
